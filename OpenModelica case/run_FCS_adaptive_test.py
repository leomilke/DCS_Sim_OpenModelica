...
writer=pd.ExcelWriter('Ada_Test.xlsx',engine='openpyxl')
p=100
pr=0

for st in ['fluc','high','low']:
    M=ModelicaSystem(fileName=dic_name+'/Full_Cooling_Sys_Auto_'+st+'_seed_input.mo',modelName='Full_Cooling_Sys_Auto_'+st+'_seed_input')
    M.buildModel()
    ...
    for seed in range(p):
        ...
        if Time_end==86400:
            CB_Run_Time=(list(M.getSolutions(['booleanToReal.y'])[0]).count(1))/(3*Time_end)*24
